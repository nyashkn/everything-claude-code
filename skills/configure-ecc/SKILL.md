...
Before prompting the user, check for an active profile:
```bash
# If CLAUDE_CONFIG_DIR is set, user-level installs go there (profile-aware)
USER_LEVEL_DIR="${CLAUDE_CONFIG_DIR:-$HOME/.claude}"
```
...
Options:
  - "User-level ($USER_LEVEL_DIR/)" — "Applies to all your Claude Code projects"
  - "Project-level (.claude/)" — "Applies only to the current project"
  - "Both" — "Common/shared items user-level, project-specific items project-level"
```

Store the choice as `INSTALL_LEVEL`. Set the target directory:
- User-level: `TARGET=$USER_LEVEL_DIR`
- Project-level: `TARGET=.claude` (relative to current project root)
- Both: `TARGET_USER=$USER_LEVEL_DIR`, `TARGET_PROJECT=.claude`
